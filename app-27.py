...
def backtest():
    ...
    try:
        ...
        workers = max(1, min(workers, 16))  # 1~16 제한

        def _scan_one(tk):
            """한 종목: 시세 -> 백테스트. (trades, had_data) 반환."""
            ...
            if not name:
                try:
                    ...
                    name = ""
            return tk, S.backtest_df(df, p, ticker=tk, name=name), True

        from concurrent.futures import ThreadPoolExecutor, as_completed
        with ThreadPoolExecutor(max_workers=workers) as ex:
            futures = [ex.submit(_scan_one, tk) for tk in universe]
            for fut in as_completed(futures):
                try:
                    _tk, trades, had_data = fut.result()
                except Exception:
                    scanned += 1
                    continue
                scanned += 1
