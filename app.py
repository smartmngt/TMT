...
v0818.001 변경 (현재가 NaN 버그 수정 + 일괄 조회)
  - /price: FDR가 최근일 종가를 NaN으로 주면 그대로 반환하던 버그 수정.
            → dropna()로 '마지막 유효 종가'를 집고, 유한값이 아니면 yfinance 폴백으로 넘어감.
            → 값이 없으면 price:null 로 반환(예전엔 NaN을 그대로 실어 JSON이 깨지고 앱이 파싱 실패).
  - /prices: 여러 종목 현재가 '한 번에' 조회하는 일괄 엔드포인트 신설(yfinance batch download).
             종목 100개+ 를 종목당 요청 100번 → 요청 1~2번으로. 콜드스타트 실패·속도 대폭 개선.
  - /fxrate: 환율도 dropna 적용(간헐 NaN 방어).
  - _last_valid_close() 공통 헬퍼 추가.
...
def _price_one(ticker, market):
    ...
    """
    from datetime import datetime as _dt, timedelta
    start = (_dt.now() - timedelta(days=15)).strftime("%Y-%m-%d")
...
def prices():
    ...
    if all_syms:
        try:
            import yfinance as yf
            data = yf.download(all_syms, period="10d", group_by="ticker",
