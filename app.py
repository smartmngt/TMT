...
v0831.001 변경
  - /closes: 지정한 날짜들의 종가를 한 번에 돌려주는 엔드포인트 신설.
             연도별·기간별 평가손익을 정확히 계산하려면 그 시점 종가가 필요하다.
             휴장일이면 그 이전 마지막 거래일 종가를 쓴다.
...
@app.route("/closes")
def closes():
    """지정한 날짜들의 종가를 한 번에 조회. 연도별 평가손익 계산용.

      GET /closes?items=O:US,JEPI:US,498400:KRX&dates=2023-12-29,2024-12-31
      → {"closes": {"O": {"2023-12-29": 57.44, ...}, ...}, "count": n}

    각 날짜에 대해 '그 날짜 이하의 마지막 거래일 종가'를 돌려준다(휴장일 방어).
    auto_adjust=False — 배당 조정이 들어가면 그 시점 실제 평가액과 어긋난다.
    """
    from datetime import datetime as _dt3, timedelta as _td3
    items_raw = (request.args.get("items") or "").strip()
    dates = [d.strip() for d in (request.args.get("dates") or "").split(",") if d.strip()]
    if not items_raw or not dates:
        return jsonify({"closes": {}, "count": 0})
    ...
    try:
        start = (_dt3.strptime(dates[0], "%Y-%m-%d") - _td3(days=14)).strftime("%Y-%m-%d")
        end = (_dt3.strptime(dates[-1], "%Y-%m-%d") + _td3(days=2)).strftime("%Y-%m-%d")
    except Exception:
    ...
    for t, m in pairs:
        ...
        sym_map[t] = syms
        for x in syms:
            if x not in all_syms:
                all_syms.append(x)

    data = None
    try:
        import yfinance as yf
        data = yf.download(all_syms, start=start, end=end, group_by="ticker",
                           threads=True, progress=False, auto_adjust=False)
    except Exception:
        data = None
    ...
    def series(sym):
        if data is None:
            return None
        try:
            sub = data if len(all_syms) == 1 else data[sym]
            col = sub["Close"] if "Close" in getattr(sub, "columns", []) else None
            return None if col is None else col.dropna()
        except Exception:
            return None
    ...
    return jsonify({"closes": out, "count": len(out), "dates": dates})
