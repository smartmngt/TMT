...
def _ned_apart_price(pnu, year):
    """
    공동주택가격 속성조회. 엔드포인트명이 바뀔 수 있어 후보를 순서대로 시도.
    반환: (행 리스트, 사용한 엔드포인트)
    """
    candidates = [
        "getApartHousingPriceAttr",   # 공동주택가격 속성조회
        "getApartHousingPrice",
        "getIndvdHousingPriceAttr",   # (단독) 개별주택가격 — 폴백
    ]
    last = None
    for name in candidates:
        try:
            j = _http_json(f"https://api.vworld.kr/ned/data/{name}", {
                "key": VWORLD_KEY,
...
def gongsiga_raw():
    """원본 응답 확인용. /gongsiga_raw?pnu=...&year=2026"""
    pnu = request.args.get("pnu")
    year = request.args.get("year") or str(datetime.now().year)
    if not pnu:
        return jsonify({"error": "pnu 필요"}), 400
    ...
    for name in ("getApartHousingPriceAttr", "getApartHousingPrice"):
        try:
            out["tried"][name] = _http_json(
                f"https://api.vworld.kr/ned/data/{name}",
                {"key": VWORLD_KEY, "domain": os.environ.get("VWORLD_DOMAIN", ""),
                 "pnu": pnu, "stdrYear": str(year), "format": "json",
                 "numOfRows": 20, "pageNo": 1}, timeout=15)
        except Exception as e:
            out["tried"][name] = {"error": str(e)}
    return jsonify(out)
