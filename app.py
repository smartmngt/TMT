...
def _resolve_snapshot_date(stock, want):
    """
    ticker_list / market_cap 는 '그 하루의 스냅샷'을 요구한다.
    want(종료일)가 휴장일이거나 아직 스냅샷이 확정되지 않은 미래·당일이면 빈 결과가 온다.
    -> want 부터 하루씩 과거로 내려가며 실제로 종목이 잡히는 첫 영업일을 찾는다.
    반환: (snapshot_date_str, tried_list)
    """
    from datetime import datetime as _dt, timedelta as _td
    try:
        cur = _dt.strptime(str(want), "%Y%m%d")
    except Exception:
        cur = _dt.now()
    # 미래 날짜면 오늘로 당김 (미래 스냅샷은 존재하지 않음)
    if cur.date() > _dt.now().date():
        cur = _dt.now()
    tried = []
    for _ in range(10):  # 최대 10영업일 전까지 탐색
        ds = cur.strftime("%Y%m%d")
        tried.append(ds)
        try:
            lst = stock.get_market_ticker_list(ds, market="KOSPI")
            if lst:  # 종목이 실제로 잡히는 날
                return ds, tried
        except Exception:
            pass
        cur -= _td(days=1)
    return want, tried  # 못 찾으면 원래 값
...
def build_universe(stock, markets, end, cap):
    ...
    # 종목/시총 스냅샷 기준일을 '데이터가 있는 직전 영업일'로 보정
    snap, tried = _resolve_snapshot_date(stock, end)
    diag["snapshot_date"] = snap
    diag["snapshot_requested"] = str(end)
    if snap != str(end):
        diag["snapshot_note"] = f"{end}는 스냅샷이 없어 {snap}로 보정"

    tickers = []
    for m in markets:
        try:
            got = _retry(lambda m=m: stock.get_market_ticker_list(snap, market=m), tries=3, delay=0.7)
            got = got or []
    ...
    try:
        cap_df = _retry(lambda: stock.get_market_cap(snap), tries=3, delay=0.7)
        if cap_df is None or cap_df.empty or "시가총액" not in cap_df.columns:
            raise ValueError("market_cap 스냅샷이 비어있음")
        ...
        ordered = tickers  # 시총 정렬 실패해도 종목 리스트로 진행
