...
v0807.005 변경 (자동매매/백테스트 제거)
  - /backtest, /diag, /diag2 라우트 삭제 (움 매매법 백테스트 기능 걷어냄)
  - strategy.py import 전부 제거 → strategy.py, backtest.html 파일 삭제 가능
  - 백테스트 전용 유틸(_clean/safe_jsonify) 및 pykrx 유니버스/시세 함수 삭제
  - 남은 기능: 종목검색·현재가·환율·배당·주소검색·공시가격 (자산관리 앱 코어)
...
#   - 명시 라우트(/status, /search, /price, /fxrate, /addr, /gongsiga 등)가
#     우선 매칭되므로 충돌 없음
