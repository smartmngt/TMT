...
v0807.004 변경 (종목검색 폴백 수정)
  - /search 폴백 조건을 'not out'(결과 완전 공백) → '정확한 티커 일치 없음'으로 변경.
    예) "blox" 검색 시 이름 부분일치로 Roblox(RBLX)만 뜨고, 정작 BLOX(신규 ETF)는
    FDR 상장리스트에 없어 안 잡히던 문제 해결. yfinance로 실티커 확인 후 맨 앞에 삽입.
  - _US_EXTRA에 BLOX(니콜라스 크립토 인컴 ETF) 보강 → 즉시·안정 검색(이중 안전장치).
"""
