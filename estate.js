...
.es-addr{flex:1;min-width:120px;padding:9px 11px;border-radius:10px;border:1.5px solid var(--line);
...
function viewHold(){
  ...
  const mem=hh.members.map(m=>`
    <div class="settings-row">
      <div style="flex:1"><input class="es-addr" style="font-weight:800;font-size:14px" value="${esc(m.name)}" data-esmn="${m.id}">
        <div class="es-note" style="margin-top:4px">연령별 세액공제 (60·65·70세↑)</div></div>
      <div style="white-space:nowrap;display:flex;align-items:center;gap:6px">
        <input class="es-num" type="number" value="${m.age}" data-esma="${m.id}"> <span class="es-k">세</span>
        ${hh.members.length>1?`<button class="del" data-esmd="${m.id}" style="color:var(--muted);font-size:12px;font-weight:700">삭제</button>`:''}
      </div>
    </div>`).join('');
  ...
  const props = hh.properties.length ? hh.properties.map(p=>{
    ...
    return `<div class="es-prop">
      ...
      <div class="es-line">
        ...
        <button class="es-mini" data-essearch="${p.id}">주소검색</button>
      </div>
...
function bindEstate(){
  ...
  on('[data-esseg]','onclick',el=>()=>{ seg=el.dataset.esseg; g('render')(); });
  on('[data-esyr]','onclick',el=>()=>{ taxYear=+el.dataset.esyr; g('render')(); });

  /* 구성원 */
  on('[data-esmn]','onchange',el=>()=>{ const m=hh.members.find(x=>x.id===el.dataset.esmn); if(m){m.name=el.value;save();g('render')();} });
  on('[data-esma]','onchange',el=>()=>{ const m=hh.members.find(x=>x.id===el.dataset.esma); if(m){m.age=+el.value||0;save();g('render')();} });
  on('[data-esmd]','onclick',el=>()=>{ const id=el.dataset.esmd;
    ...
    save(); g('render')(); });
  on('[data-esmadd]','onclick',()=>()=>{ hh.members.push({id:uid(),name:'배우자',age:50}); save(); g('render')(); });
  ...
  on('[data-espd]','onclick',el=>()=>{ if(!confirm('이 주택을 삭제할까요?'))return;
    hh.properties=hh.properties.filter(p=>p.id!==el.dataset.espd); save(); g('render')(); });
  on('[data-espp]','onchange',el=>()=>{ P(el.dataset.espp).primary=el.checked; save(); g('render')(); });
  on('[data-espj]','onchange',el=>()=>{ P(el.dataset.espj).adjusted=el.checked; save(); });
  on('[data-espq]','onchange',el=>()=>{ P(el.dataset.espq).acquire=el.value; save(); g('render')(); });
  on('[data-espl]','onchange',el=>()=>{ P(el.dataset.espl).liveStart=el.value; save(); g('render')(); });
  on('[data-esown]','onchange',el=>()=>{ const [pid,mid]=el.dataset.esown.split('|'); const p=P(pid);
    ...
    if(sh<=0) delete p.owners[mid]; else p.owners[mid]=sh; save(); g('render')(); });
  on('[data-espr]','onchange',el=>()=>{ const [pid,y]=el.dataset.espr.split('|'); const p=P(pid);
    p.prices=p.prices||{}; p.prices[y]=(parseFloat(el.value)||0)*억; save(); g('render')(); });
  ...
  on('[data-esrec]','onchange',el=>()=>{ const [y,k]=el.dataset.esrec.split('|');
    hh.records[y]=hh.records[y]||{}; hh.records[y][k]=(parseFloat(el.value)||0)*만; save(); g('render')(); });
  on('[data-esgrow]','onchange',el=>()=>{ growth=parseFloat(el.value)||0; g('render')(); });
  on('[data-eswtog]','onclick',()=>()=>{ whatOn=!whatOn; g('render')(); });
  on('[data-eswpub]','onchange',el=>()=>{ e.wPub=parseFloat(el.value)||15; save(); g('render')(); });
  on('[data-eswowner]','onchange',el=>()=>{ e.wOwner=el.value; save(); g('render')(); });
}

/* 세대 설정 시트 */
function openHHedit(){
  const e=E(), hh=HH();
  g('sheet')(`<h2>세대 설정</h2><div class="hint">세대 이름을 바꾸거나 삭제할 수 있어요.</div>
  ...
  $q('#es-hs').onclick=()=>{ const v=$q('#es-hn').value.trim(); if(!v) return toast('이름을 입력하세요');
    hh.name=v; save(); g('close')(); g('render')(); toast('저장됨'); };
  const del=$q('#es-hd');
  if(del) del.onclick=()=>{ if(!confirm(`'${hh.name}' 세대를 삭제할까요?\n주택·기록도 함께 삭제됩니다.`)) return;
    ...
    save(); g('close')(); g('render')(); toast('삭제됨'); };
}
...
function openAddr(pid){
  ...
  g('sheet')(`<h2>주소 검색</h2><div class="hint">도로명·지번·단지명으로 찾으면 공시가격을 자동으로 불러와요.</div>
    <div class="field"><label>검색어</label>
      <div style="display:flex;gap:8px"><input id="es-akw" placeholder="예: 반포동 18-1 · 래미안퍼스티지"
        value="${esc(p.address||'')}" autocomplete="off" style="flex:1">
        <button class="btn" id="es-ago" style="padding:13px 18px">검색</button></div></div>
    <div id="es-alist" style="max-height:44vh;overflow:auto;margin-bottom:12px"></div>
  ...
  const kw=$q('#es-akw');
  $q('#es-ago').onclick=doSearch;
  kw.onkeydown=ev=>{ if(ev.key==='Enter') doSearch(); };

  async function doSearch(){
    const q=kw.value.trim(); const L=$q('#es-alist');
    if(!q) return;
    L.innerHTML='<div class="search-hint">검색 중…</div>';
    const res=await g('apiGet')('/addr?keyword='+encodeURIComponent(q));
    if(!res){ L.innerHTML='<div class="search-hint">검색 실패 · 서버(/addr) 확인</div>'; return; }
    if(res.error){ L.innerHTML=`<div class="search-hint">${esc(res.error)}</div>`; return; }
  ...
  function pick(i){
    ...
    const pnu=makePNU(it);
    $q('#es-alist').innerHTML=`<div class="search-item" style="background:var(--green-t);border-radius:10px">
      <div style="flex:1;min-width:0"><div style="font-weight:800;font-size:13.5px">${esc(it.roadAddr||'')}</div>
      <div style="font-size:11.5px;color:var(--green);font-weight:700;margin-top:3px">PNU ${pnu||'조립 실패'}</div></div></div>`;
    $q('#es-adh').style.display='block';
  }
  $q('#es-aok').onclick=async()=>{
    if(!addrPick) return toast('주소를 선택하세요');
    const p=HH().properties.find(x=>x.id===addrPid);
    p.address=addrPick.roadAddr||addrPick.jibunAddr||p.address;
    if(addrPick.bdNm && p.name==='새 주택') p.name=addrPick.bdNm;
    p.pnu=makePNU(addrPick);
    p.dong=$q('#es-adong').value.trim();
    p.ho=$q('#es-aho').value.trim();
    save(); g('close')(); g('render')();
    if(p.pnu) fetchPrice(p.id);
  };
}
...
function attach(){
  ...
  btn.onclick=()=>{ window.tab='estate'; g('render')(); window.scrollTo(0,0); };
  const setBtn=nav.querySelector('[data-tab="settings"]');
  nav.insertBefore(btn, setBtn);

  /* render() 감싸기 */
  const orig=window.render;
  window.render=function(){
    if(window.tab==='estate'){
      ...
      bindEstate();
      try{ localStorage.setItem('tmt.ui.estate','1'); }catch(err){}
      return;
    }
  ...
  /* 마지막에 부동산 탭이었으면 복원 */
  try{ if(localStorage.getItem('tmt.ui.estate')==='1'){ window.tab='estate'; window.render(); } }catch(err){}
}
