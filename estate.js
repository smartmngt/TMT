/* =============================================================================
   부동산 보유세 모듈  estate.js   v0808.001
   -----------------------------------------------------------------------------
   ...
   - 주소검색은 카카오(다음) 우편번호 서비스(클라이언트 전용, 키 불필요) 사용
   - 공시가격은 realtyprice.kr(국토부 공식) 버튼으로 열어 사용자가 직접 조회·입력
...
/* 거주기간 — 이사·재입주 등으로 끊긴 구간이 있을 수 있어 여러 개를 합산한다 (읽기 전용, 구버전 liveStart 자동 변환) */
function livePeriods(p){
  if(Array.isArray(p.livePeriods) && p.livePeriods.length) return p.livePeriods;
  if(p.liveStart) return [{start:p.liveStart, end:''}];
  return [];
}
function liveMonths(p,y){
  const cutoff=y*12+6; let total=0;
  livePeriods(p).forEach(pd=>{
    const s=ym(pd.start); if(s==null) return;
    let e = pd.end ? ym(pd.end) : cutoff;
    e = Math.min(e, cutoff);
    if(e>s) total += (e-s);
  });
  return total;
}
const livedY = (p,y)=>{ if(!p.primary) return 0; return Math.max(0, Math.floor(liveMonths(p,y)/12)); };

/* ============================================================
   2. 상태
   ============================================================ */
const REC_YEARS=[2024,2025,2026];
let seg='hold', taxYear=2026, whatOn=false, growth=3, memOpen=false, rateOpen=false;
...
.es-arrow{width:30px;height:30px;border-radius:9px;background:var(--card);border:1.5px solid var(--line);
  color:var(--ink2);display:grid;place-items:center;flex:none;transition:transform .2s}
.es-arrow:active{background:var(--bg)}
.es-arrow svg{width:16px;height:16px;transition:transform .2s}
.es-arrow.open svg{transform:rotate(180deg)}
...
function viewHold(){
  ...
  const memSummary = hh.members.map(m=>`${esc(m.name)} ${ageAt(m,2026)}세`).join(' · ');
  const memBlock = memOpen
    ? `<div class="setcard">${mem}
    ...
    : `<div class="setcard"><div class="settings-row" style="border:none"><div class="sk">${esc(memSummary)}</div></div></div>`;
  ...
  const props = hh.properties.length ? hh.properties.map(p=>{
    ...
    const lps = livePeriods(p);
    const lpRows = lps.map((pd,i)=>`<div class="es-line" style="margin-bottom:6px">
        <input class="es-date" type="month" value="${pd.start||''}" data-eslps="${p.id}|${i}">
        <span class="es-k">~</span>
        <input class="es-date" type="month" value="${pd.end||''}" placeholder="현재까지" data-eslpe="${p.id}|${i}">
        ${lps.length>1?`<button class="es-mini" style="padding:6px 9px" data-eslpdel="${p.id}|${i}">삭제</button>`:''}
      </div>`).join('');
    return `<div class="es-prop">
      ...
      ${(p.dong||p.ho)?`<div class="es-note" style="margin:-4px 0 8px">${p.dong?`${esc(p.dong)}동 `:''}${p.ho?`${esc(p.ho)}호`:''} 확인됨</div>`:''}
      ...
      <div class="es-line">
        ...
        <div style="align-self:flex-end;font-size:12px;font-weight:700;color:var(--ink2)">
          보유 <b style="color:var(--green)">${heldY(p,2026)}년</b> · 거주 <b style="color:var(--green)">${livedY(p,2026)}년</b> (합산)</div>
      </div>
      <div class="es-k" style="margin:2px 0 6px">거주기간 <span style="font-weight:600">· 이사·재입주로 끊겼으면 구간을 나눠 넣으세요</span></div>
      ${lpRows || '<div class="es-note" style="margin:-2px 0 8px">아직 없음 · 실거주 체크 시 아래에서 추가</div>'}
      <button class="es-mini" style="margin-bottom:9px" data-eslpadd="${p.id}">+ 거주기간 추가</button>
  ...
  return `<div class="sec" style="display:flex;align-items:center;gap:4px">
      <span style="flex:1">세대 구성원</span>
      <button class="es-arrow ${memOpen?'open':''}" data-esmemtog="1" aria-label="${memOpen?'접기':'펼치기'}"><svg viewBox="0 0 24 24" fill="none"><path d="M6 9l6 6 6-6" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg></button>
    </div>
    ${memBlock}
    ...
    <div class="es-hint2">※ 2026.8.3 발표 개편안 기준(국회 통과 전) · 종부세 세액공제는 현행 <b>보유기간</b>,
      2028년 개편 후 <b>거주기간</b> 기준이라 취득일과 거주기간 구간을 정확히 넣어야 합니다.</div>`;
}
...
function viewTax(){
  ...
  const r=compute(hh,taxYear);
  const yIdx=years.indexOf(taxYear);
  const prevYear = yIdx>0 ? years[yIdx-1] : null;
  const prevR = prevYear!=null ? compute(hh,prevYear) : null;
  const d = prevR ? r.total-prevR.total : 0;
  ...
  return `<div class="es-yr">${years.map(y=>`<button class="${y===taxYear?'on':''}" data-esyr="${y}">${y}${y>=2027?' 개편':''}</button>`).join('')}</div>
    <div class="sumcard"><div class="glow"></div>
      ...
      ${prevR?`<div style="margin-top:8px;text-align:right;font-size:12.5px;font-weight:700;color:${d>=0?'#FF8B8B':'#3FE0A0'}">
        ${prevYear}년 대비 ${d>=0?'+':'−'}${만원(Math.abs(d))} (${prevR.total?((d/prevR.total)*100).toFixed(0):0}%)</div>`:''}
    </div>
    ...
    <div class="sec" style="display:flex;align-items:center;gap:4px">
      <span style="flex:1">계산식 · 세율표</span>
      <button class="es-arrow ${rateOpen?'open':''}" data-esratetog="1" aria-label="${rateOpen?'접기':'펼치기'}"><svg viewBox="0 0 24 24" fill="none"><path d="M6 9l6 6 6-6" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg></button>
    </div>
    ${rateOpen?renderRateTables():''}`;
}

/* ---- 계산식 · 세율표 (EST.cfg에서 그대로 생성 — 개정 시 cfg만 고치면 자동 반영) ---- */
function renderRateTables(){
  const C=EST.cfg;
  const fmtEok=v=>{ const n=Math.round(v/억*10)/10; return n.toString(); };
  const pctv=x=>{ const v=Math.round(x*100000)/1000; return (v%1===0? v.toFixed(0): v.toString())+'%'; };
  const bracketRows=br=>{ let prev=0; return br.map(b=>{
      const lo=prev, hi=b.upTo; prev=b.upTo;
      const label = hi===Infinity? `${fmtEok(lo)}억 초과` : lo===0? `${fmtEok(hi)}억 이하` : `${fmtEok(lo)}~${fmtEok(hi)}억`;
      return `<tr><td>${label}</td><td class="num">${pctv(b.rate)}</td></tr>`; }).join(''); };
  const bracketTable=(title,br)=>`<div class="es-k" style="margin:12px 0 4px">${title}</div>
    <table class="es-tb"><thead><tr><th>과세표준 구간</th><th>세율</th></tr></thead><tbody>${bracketRows(br)}</tbody></table>`;

  return `<div class="setcard" style="padding:14px 16px">
    <div class="es-hint2" style="margin:0 0 4px"><b>① 재산세</b><br>
      과세표준 = 공시가격 × 공정시장가액비율(1세대1주택 45% · 그 외 60%)<br>
      산출세액 = 과세표준에 누진세율 적용 + 도시지역분(과세표준×${pctv(C.도시분)})<br>
      납부세액 = 산출세액 + 지방교육세(산출세액의 ${pctv(C.교육세)})</div>
    ${bracketTable('표준세율 (일반)', C.jaeStd)}
    ${bracketTable('특례세율 (1세대1주택 · 공시가 9억 이하)', C.jaeSpc)}

    <div class="es-hint2" style="margin:16px 0 4px"><b>② 종합부동산세</b><br>
      과세표준 = (인별 공시가격 합산 − 기본공제) × 공정시장가액비율<br>
      산출세액 = 과세표준에 누진세율 적용 − 재산세 중복분 공제<br>
      결정세액 = 산출세액 − 세액공제(연령+보유·거주기간, 최대 ${pctv(C.cap)}) − 세부담상한 초과분<br>
      납부세액 = 결정세액 + 농어촌특별세(결정세액의 ${pctv(C.농특)})</div>
    ${bracketTable('1~2주택 · 현행(~2026)', C.jong.cur_1_2)}
    ${bracketTable('3주택 이상 · 현행(~2026)', C.jong.cur_3)}
    ${bracketTable('1~2주택 · 2027년 개편', C.jong.y27_1_2)}
    ${bracketTable('3주택 이상 · 2027년 개편', C.jong.y27_3)}
    ${bracketTable('2028년 개편 완료 (주택수 무관 단일세율)', C.jong.y28)}

    <div class="es-k" style="margin:16px 0 4px">기본공제 (종부세)</div>
    <table class="es-tb"><thead><tr><th>구분</th><th>현행(~2026)</th><th>개편(2027~)</th></tr></thead><tbody>
      <tr><td>1세대1주택 실거주</td><td class="num">${fmtEok(C.ded.cur.live)}억</td><td class="num">${fmtEok(C.ded.re.live)}억</td></tr>
      <tr><td>1주택 비거주</td><td class="num">${fmtEok(C.ded.cur.nonlive)}억</td><td class="num">${fmtEok(C.ded.re.nonlive)}억</td></tr>
      <tr><td>다주택</td><td class="num">${fmtEok(C.ded.cur.multi)}억</td><td class="num">${fmtEok(C.ded.re.multi)}억</td></tr>
    </tbody></table>

    <div class="es-k" style="margin:16px 0 4px">공정시장가액비율 (종부세)</div>
    <table class="es-tb"><thead><tr><th>연도</th><th>1주택</th><th>다주택</th></tr></thead><tbody>
      <tr><td>2026</td><td class="num">${pctv(C.fair[2026].live)}</td><td class="num">${pctv(C.fair[2026].multi)}</td></tr>
      <tr><td>2027</td><td class="num">${pctv(C.fair[2027].live)}</td><td class="num">${pctv(C.fair[2027].multi)}</td></tr>
      <tr><td>2028~</td><td class="num">${pctv(C.fair[2028].live)}</td><td class="num">${pctv(C.fair[2028].multi)}</td></tr>
    </tbody></table>

    <div class="es-k" style="margin:16px 0 4px">세액공제 — 연령 (1세대1주택자만)</div>
    <table class="es-tb"><thead><tr><th>연령</th><th>공제율</th></tr></thead><tbody>
      ${C.ageCr.map(([t,rr])=>`<tr><td>만 ${t}세 이상</td><td class="num">${pctv(rr)}</td></tr>`).join('')}
    </tbody></table>

    <div class="es-k" style="margin:16px 0 4px">세액공제 — 보유·거주기간 <span style="font-weight:600">(현행: 보유기간 · 2028~: 거주기간)</span></div>
    <table class="es-tb"><thead><tr><th>기간</th><th>공제율</th></tr></thead><tbody>
      ${C.holdCr.map(([t,rr])=>`<tr><td>${t}년 이상</td><td class="num">${pctv(rr)}</td></tr>`).join('')}
    </tbody></table>

    <div class="es-hint2" style="margin-top:12px">세액공제 합산 한도 <b>${pctv(C.cap)}</b> · 세부담상한(전년 납부세액 대비) 현행 <b>${(C.burden.cur*100).toFixed(0)}%</b> · 개편(2027~) <b>${(C.burden.re*100).toFixed(0)}%</b></div>
  </div>`;
}
...
function bindEstate(){
  ...
  on('[data-esmemtog]','onclick',()=>()=>{ memOpen=!memOpen; window.render(); });
  on('[data-esratetog]','onclick',()=>()=>{ rateOpen=!rateOpen; window.render(); });
  ...
  on('[data-espadd]','onclick',()=>()=>{
    hh.properties.push({id:uid(),name:'새 주택',address:'',owners:{[hh.members[0].id]:100},
      primary:hh.properties.length===0,acquire:'',livePeriods:[],prices:{}});
    save(); window.render(); });
  const P=id=>hh.properties.find(p=>p.id===id);
  const LP=p=>{ if(!Array.isArray(p.livePeriods)) p.livePeriods = p.liveStart?[{start:p.liveStart,end:''}]:[]; return p.livePeriods; };
  ...
  on('[data-eslps]','onchange',el=>()=>{ const [pid,idx]=el.dataset.eslps.split('|'); const lp=LP(P(pid));
    if(lp[idx]){ lp[idx].start=el.value; save(); window.render(); } });
  on('[data-eslpe]','onchange',el=>()=>{ const [pid,idx]=el.dataset.eslpe.split('|'); const lp=LP(P(pid));
    if(lp[idx]){ lp[idx].end=el.value; save(); window.render(); } });
  on('[data-eslpdel]','onclick',el=>()=>{ const [pid,idx]=el.dataset.eslpdel.split('|'); const lp=LP(P(pid));
    lp.splice(+idx,1); save(); window.render(); });
  on('[data-eslpadd]','onclick',el=>()=>{ const lp=LP(P(el.dataset.eslpadd));
    lp.push({start:'',end:''}); save(); window.render(); });
...
/* ============================================================
   7. 주소검색 (카카오 우편번호) / 공시가격은 realtyprice.kr 수동 조회
   ============================================================ */
/* ============================================================
...
function onDaumPick(d){
  try{
    ...
    if(!p.pnu){ toast('주소 저장됨 · PNU 조립 실패, 공시가는 realtyprice.kr에서 직접 확인하세요'); return; }
    if(p.isApt) askDongHo(p.id); else toast('주소 저장됨 · 공시가격 조회 버튼으로 realtyprice.kr에서 확인하세요');
...
function askDongHo(pid){
  ...
  window.sheet(`<h2>동·호 입력</h2><div class="hint">${esc(p.address||'')}<br>
      아파트는 동·호까지 넣어두면 realtyprice.kr에서 조회할 때 헷갈리지 않아요.</div>
    <div class="row2">
    ...
    <div class="sheet-actions">
      ...
      <button class="btn" id="es-dhok">저장</button></div>`);
  const ok=$q('#es-dhok'), skip=$q('#es-skip');
  if(ok) ok.onclick=()=>{
    const dv=$q('#es-adong'), hv=$q('#es-aho');
    p.dong=dv?dv.value.trim():''; p.ho=hv?hv.value.trim():'';
    save(); window.close(); window.render(); toast('저장됨 · 공시가격 조회 버튼으로 확인하세요');
  };
  if(skip) skip.onclick=()=>{ window.close(); };
}
