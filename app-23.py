...
def gongsiga():
    """
    공시가 조회. GET /gongsiga?pnu=1165010700106500000&year=2026&dong=101&ho=1203
    응답: {year, price, matched:{...}}   price 단위 = 원
    """
    ...
    try:
        j = _http_json("https://api.vworld.kr/req/data", {
            "service": "data", "request": "GetFeature", "format": "json",
            "data": "LT_C_APTHOSPRC", "attrFilter": f"pnu:=:{pnu}",
            "size": 1000, "page": 1,
            "key": VWORLD_KEY, "domain": os.environ.get("VWORLD_DOMAIN", ""),
        }, timeout=15)

        feats = (((j.get("response") or {}).get("result") or {})
                 .get("featureCollection") or {}).get("features") or []
        if not feats:
            return jsonify({"year": year, "price": 0, "note": "PNU 매칭 없음",
                            "raw_status": (j.get("response") or {}).get("status")})

        cand = [f.get("properties", {}) for f in feats]

        # 동/호로 좁히기 (있을 때만)
        if dong:
            f1 = [p for p in cand
                  if _digits(p.get("dongNm") or p.get("bldgNm")) == _digits(dong)]
            if f1:
                cand = f1
        if ho:
            f2 = [p for p in cand if _digits(p.get("hoNm")) == _digits(ho)]
            if f2:
                cand = f2

        # 공시가격 필드 자동 탐색 (데이터셋 버전마다 필드명이 달라 유연하게)
        p0 = cand[0]
        price_val, key_used = 0, None
        for k, v in p0.items():
            if any(t in k.lower() for t in ("prc", "price", "amt")) and _digits(v):
                price_val, key_used = int(_digits(v)), k
                break

        return jsonify({
            "year": year,
            "price": price_val,
            "matched": {"dong": p0.get("dongNm"), "ho": p0.get("hoNm"), "field": key_used},
            "count": len(cand),
        })
    except Exception as e:
...
@app.route("/juso", methods=["GET", "POST"])
def juso_popup():
    """
    행정안전부 '도로명주소 팝업 API' 연동 페이지 (쇼핑몰에서 흔히 보는 그 주소찾기 화면).

    흐름:
      1) 앱이 window.open('/juso') → 이 페이지가 juso.go.kr로 자동 POST (검색화면 표시)
      2) 사용자가 주소 선택 → juso.go.kr이 이 페이지로 다시 POST (inputYn=Y)
      3) 이 페이지가 opener.jusoCallBack(데이터) 호출 후 자기 자신을 닫음

    환경변수: JUSO_POPUP_KEY (팝업API 승인키 · 검색API 키와 별개)
              미설정 시 juso.go.kr 공개 테스트키로 동작 (운영은 반드시 발급받을 것)
    """
    import json as _json

    key = os.environ.get("JUSO_POPUP_KEY") or "TESTJUSOGOKR"
    f = request.form

    # ---- 2단계: juso.go.kr이 결과를 돌려준 경우 ----
    if request.method == "POST" and f.get("inputYn") == "Y":
        payload = {
            "roadAddr":  f.get("roadFullAddr") or f.get("roadAddrPart1") or "",
            "jibunAddr": f.get("jibunAddr") or "",
            "bdNm":      f.get("bdNm") or "",
            "zipNo":     f.get("zipNo") or "",
            "admCd":     f.get("admCd") or "",     # 법정동코드 10자리
            "mtYn":      f.get("mtYn") or "0",     # 산 여부
            "lnbrMnnm":  f.get("lnbrMnnm") or "0",  # 지번 본번
            "lnbrSlno":  f.get("lnbrSlno") or "0",  # 지번 부번
            "bdKdncd":   f.get("bdKdcd") or "",     # 공동주택 여부(1)
        }
        html = """<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8">
<title>주소 선택 완료</title></head><body style="font-family:sans-serif;padding:24px;text-align:center">
<p>주소를 가져오는 중…</p>
<script>
(function(){
  var data = %s;
  try{
    ...
    if(t && t.jusoCallBack) t.jusoCallBack(data);
  }catch(e){}
  try{ window.close(); }catch(e){}
  setTimeout(function(){ document.body.innerHTML =
    '<p>주소가 전달되었습니다. 이 창을 닫아주세요.</p>'; }, 400);
})();
</script></body></html>""" % _json.dumps(payload, ensure_ascii=False)
        return html, 200, {"Content-Type": "text/html; charset=utf-8"}

    # ---- 1단계: juso.go.kr 검색화면으로 자동 POST ----
    ua = (request.headers.get("User-Agent") or "").lower()
    is_mobile = any(k in ua for k in ("android", "iphone", "ipad", "mobile"))
    action = ("https://business.juso.go.kr/addrlink/addrMobileLinkUrl.do"
              if is_mobile else
              "https://business.juso.go.kr/addrlink/addrLinkUrl.do")
    return_url = request.base_url  # 이 페이지 자신

    html = """<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>주소 찾기</title></head>
<body style="margin:0;font-family:sans-serif">
<form name="form" method="post" action="%s">
  <input type="hidden" name="confmKey" value="%s">
  <input type="hidden" name="returnUrl" value="%s">
  <input type="hidden" name="resultType" value="4">
</form>
<script>document.form.submit();</script>
</body></html>""" % (action, key, return_url)
    return html, 200, {"Content-Type": "text/html; charset=utf-8"}


@app.route("/gongsiga_raw")
def gongsiga_raw():
    """필드명 확인용 임시 라우트. /gongsiga_raw?pnu=... 로 원본 응답 확인 후 삭제 가능."""
    pnu = request.args.get("pnu")
    if not pnu:
        return jsonify({"error": "pnu 필요"}), 400
    if not VWORLD_KEY:
    ...
    try:
        j = _http_json("https://api.vworld.kr/req/data", {
            "service": "data", "request": "GetFeature", "format": "json",
            "data": "LT_C_APTHOSPRC", "attrFilter": f"pnu:=:{pnu}",
            "size": 10, "page": 1, "key": VWORLD_KEY,
        }, timeout=15)
        return jsonify(j)
    except Exception as e:
        return _err("gongsiga_raw", e)
