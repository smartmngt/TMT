...
def aggregate(trades: list) -> dict:
    ...
    # 순차복리(equity/MDD)는 거래 순서에 의존하므로, 항상 (날짜, 종목) 순으로 고정.
    # 병렬 스캔 등으로 입력 순서가 달라도 결과가 재현되도록 보장.
    sort_cols = [c for c in ("date", "ticker") if c in tdf.columns]
    if sort_cols:
        tdf = tdf.sort_values(sort_cols, kind="mergesort").reset_index(drop=True)

    # 기간(연) 추정: 첫~마지막 거래 날짜 간격
    def _years_span():
        try:
            ds = pd.to_datetime(tdf["date"])
            span = (ds.max() - ds.min()).days / 365.25
    ...
    def stat(col):
        ...
        sk = round(float(sk), 3) if sk == sk else None

        # 총수익(순차복리): equity 마지막값 - 1. '한 번에 한 종목 몰빵' 가정.
        # 신호가 매우 많으면 이 값은 비현실적(과장 또는 -100%)일 수 있어 참고용.
        total_ret = float(eq[-1] - 1.0) if len(eq) else 0.0
        avg = float(r.mean())
        trades_per_year = len(r) / yrs
        ...
        return {
            ...
            "skew": sk,
            "mdd": round(mdd, 4),
            "total_ret": round(total_ret, 4),          # 순차복리 총수익(참고, 몰빵가정)
            "years": round(yrs, 2),
    ...
    def _score(s):
        """
        종합점수: 수익성 × 표본신뢰도 × 승률보정.
        - 평균수익(avg)이 핵심. 음수면 점수도 음수.
        - 표본이 적으면(20 미만) 강하게 감점: 통계적으로 못 믿는 조합 배제.
        - 승률은 0.5 기준 보정.
        - MDD가 크면 소폭 감점.
        점수 자체는 상대 비교용(절대값 의미 없음).
        """
        n = s["n"]
        if n <= 0:
            return 0.0
        import math as _m
        avg = s["avg"]
        wr_adj = 0.5 + (s["win_rate"] - 0.5) * 1.2
        wr_adj = max(0.1, wr_adj)
        # 표본 신뢰: 20개 미만이면 급격히 감점(sigmoid형), 이후 완만히 증가
        conf = (n / (n + 20.0))          # n=20->0.5, n=80->0.8, n=5->0.2
        sample = _m.sqrt(min(n, 300)) * conf
        dd_pen = 1.0 / (1.0 + abs(s["mdd"]) * 1.5)
        # 파산 조합(MDD -50% 이하)은 실전 불가 -> 강한 감점
        if s["mdd"] <= -0.5:
            dd_pen *= 0.15
        return round(avg * sample * wr_adj * dd_pen * 100, 2)
    ...
    return {
        ...
        "A": a_stat,  # 영상대로: 손절 없음
        "B": b_stat,  # 리스크관리: 손절
        "score_A": _score(a_stat),
        "score_B": _score(b_stat),
