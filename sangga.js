...
function viewSangga(){
  ...
  html+=`<div class="sec sg-sec">① 취득 · 증여<button class="sbtn" data-sgeditinfo="1">수정</button></div>
    ...
    (burden>0?`<div class="settings-row"><div class="sk" style="color:var(--muted)">취득 총부담(증여세+취득비용)</div><div class="num" style="font-weight:800;color:var(--muted)">${fmtKRW(burden)}</div></div>`:'')+
  ...
  const capital=s.ledger.txns.filter(t=>t.capital&&(+t.amount||0)<0).reduce((a,t)=>a+(-t.amount),0);
  ...
  html+=`<div style="text-align:center;color:var(--muted);font-size:11px;font-weight:600;margin:10px 0 18px">상가 관리 · v0817.7</div>`;
  return html;
}
...
function writeInfoForm(){
  ...
  const gl=numOf($q('#sg-gland').value), gb=numOf($q('#sg-gbldg').value), ded=numOf($q('#sg-ded').value);
  ...
  s.acq=Object.assign({}, a, { acqDate:$q('#sg-acqd').value, giftLand:gl, giftBldg:gb, giftDeduction:ded,
    declared:g.val, giftTax:g.tax, memo:$q('#sg-memo').value.trim() });
}
function openInfo(){
  ...
  window.sheet(`<h2>상가 정보 · 취득(증여)</h2>
    ...
    <div class="sec" style="margin:8px 2px 8px">증여세 계산</div>
    <div class="row2"><div class="field"><label>토지가액(증여용, 원)</label><input id="sg-gland" type="text" inputmode="numeric" value="${commaFmt(gl)}"></div>
      <div class="field"><label>건물가액(증여용, 원)</label><input id="sg-gbldg" type="text" inputmode="numeric" value="${commaFmt(gb)}"></div></div>
    <div class="field"><label>증여재산공제(원)</label><input id="sg-ded" type="text" inputmode="numeric" value="${commaFmt(ded0)}"><div class="hint">직계 성년 5천만 · 배우자 6억 · 미성년 2천만 (10년 합산)</div></div>
    <div class="calc-preview on sg-acqbox" id="sg-giftcalc">${giftBreakdownHTML(numOf(gl),numOf(gb),ded0)}</div>
    <div class="sec" style="margin:12px 2px 8px">취득세</div>
    <button class="setcard" id="sg-acqtaxbtn" style="width:100%;display:block;text-align:left;color:inherit;margin-bottom:0">
      <div style="display:flex;justify-content:space-between;align-items:center;padding:12px 2px">
        <div><div class="sk">취득세 공과금 · 취득비용</div><div class="sd">터치 → 토지·건물가 입력, 자동 계산</div></div>
        <div style="text-align:right"><div class="num" style="font-weight:800;font-size:16px">${a.acqTax?fmtKRW(+a.acqTax):'입력'}</div><div style="color:var(--green);font-size:11px;font-weight:800">상세 ▸</div></div>
      </div></button>
  ...
  ['sg-gland','sg-gbldg','sg-ded'].forEach(id=>wireComma($q('#'+id)));
  const recalc=()=>{ $q('#sg-giftcalc').innerHTML=giftBreakdownHTML(numOf($q('#sg-gland').value),numOf($q('#sg-gbldg').value),numOf($q('#sg-ded').value)); };
  ['sg-gland','sg-gbldg','sg-ded'].forEach(id=>$q('#'+id).addEventListener('input',recalc));
  $q('#sg-findaddr').onclick=()=>{ writeInfoForm(); save(); openAddr(); };
  $q('#sg-acqtaxbtn').onclick=()=>{ writeInfoForm(); save(); openAcqTax(); };
...
function openAcqTax(){
  ...
  $q('#at-cancel').onclick=()=>openInfo();
  $q('#at-save').onclick=()=>{ writeAcqTax(); save(); openInfo(); toast('취득세 저장됨'); };
}
