/* =============================================================================
   상가 관리 모듈  sangga.js   v0817.3
   -----------------------------------------------------------------------------
   index.html은 딱 한 줄만 추가 (estate.js 다음 줄):
        <script src="sangga.js"></script>
   - 데이터는 db.sangga 에 저장 → TMT 백업 자동 포함 (계약서 파일만 IndexedDB 별도)
   - 상가 1개: ① 취득(증여) ② 보유 구간(임차/공실) ③ 통장(0원부터)
   - v0817.3: 카카오 주소검색 · 토지/건물가 입력 → 증여세·취득세 계산 흐름 표시
   ============================================================================= */
(function(){
"use strict";
...
/* ---------- 증여세·취득세 계산 (2026 현행) ---------- */
/* 증여세 누진세율: 1억↓10% / 5억↓20%(-1천만) / 10억↓30%(-6천만) / 30억↓40%(-1.6억) / 초과50%(-4.6억) */
...
function calcAcq(land, bldg, ded){
  const val=land+bldg;                                   // 증여재산가액 = 신고가액 = 취득세 과표
  const giftBase=Math.max(0, val-ded);                   // 증여세 과세표준
  const giftGross=giftTaxGross(giftBase);                // 산출세액
  const reportCr=Math.floor(giftGross*0.03);             // 신고세액공제 3%
  const giftTax=Math.max(0, giftGross-reportCr);         // 납부 증여세
  const acqMain=Math.floor(val*0.035);                   // 취득세 본세 3.5%
  const acqRural=Math.floor(val*0.002);                  // 농어촌특별세 0.2%
  const acqEdu=Math.floor(val*0.003);                    // 지방교육세 0.3%
  const acqTax=acqMain+acqRural+acqEdu;                  // 합계 4.0%
...
function viewSangga(){
  ...
  html+=`<div style="text-align:center;color:var(--muted);font-size:11px;font-weight:600;margin:18px 0">상가 관리 · v0817.3</div>`;
  return html;
}
