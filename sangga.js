/* =============================================================================
   상가 관리 모듈  sangga.js   v0817.2
   -----------------------------------------------------------------------------
   index.html은 딱 한 줄만 추가 (estate.js 다음 줄):
        <script src="sangga.js"></script>
   - 데이터는 db.sangga 에 저장 → TMT 백업에 자동 포함 (계약서 파일만 IndexedDB 별도)
   - 상가 1개: ① 취득(증여) ② 보유 구간(임차/공실) ③ 통장(0원부터)
   - v0817.2: 금액 콤마입력 · 임차인 계좌번호 · 관리비 실비토글 · 관리비 월별 입력버튼
   ============================================================================= */
(function(){
"use strict";
...
/* 관리비 다음 입력 월 (마지막 관리비 다음 달, 없으면 4월부터) */
...
function viewSangga(){
  ...
  html+=`<div style="text-align:center;color:var(--muted);font-size:11px;font-weight:600;margin:18px 0">상가 관리 · v0817.2</div>`;
  return html;
}
...
function openInfo(){
  ...
  window.sheet(`<h2>상가 정보 · 취득(증여)</h2>
    ...
    <div class="row2"><div class="field"><label>취득일</label><input id="sg-acqd" type="date" value="${a.acqDate||''}"></div>
      <div class="field"><label>증여 신고가액(원)</label><input id="sg-decl" type="text" inputmode="numeric" value="${commaFmt(a.declared)}"></div></div>
    <div class="row2"><div class="field"><label>증여세(원)</label><input id="sg-gift" type="text" inputmode="numeric" value="${commaFmt(a.giftTax)}"></div>
      <div class="field"><label>취득세(원)</label><input id="sg-atax" type="text" inputmode="numeric" value="${commaFmt(a.acqTax)}"></div></div>
    <div class="field"><label>기타 수수료·부대(원)</label><input id="sg-fee" type="text" inputmode="numeric" value="${commaFmt(a.fee)}"></div>
  ...
  ['sg-decl','sg-gift','sg-atax','sg-fee'].forEach(id=>wireComma($q('#'+id)));
  $q('#sg-cancel').onclick=window.close;
  $q('#sg-save').onclick=()=>{
    s.info.nickname=$q('#sg-nick').value.trim(); s.info.address=$q('#sg-addr').value.trim();
    s.acq={ acqDate:$q('#sg-acqd').value, declared:numOf($q('#sg-decl').value), giftTax:numOf($q('#sg-gift').value),
      acqTax:numOf($q('#sg-atax').value), fee:numOf($q('#sg-fee').value), memo:$q('#sg-memo').value.trim() };
...
function openSeg(id){
  ...
  $q('#sg-save').onclick=async()=>{
    ...
    if(type==='lease'){ Object.assign(data,{
      tenant:$q('#sg-tenant').value.trim(), account:$q('#sg-account').value.trim(),
      deposit:numOf($q('#sg-dep').value), rent:numOf($q('#sg-rent').value), vatSeparate:vat,
      mgmtActual:mgmtActual, mgmt: mgmtActual? 0 : numOf($q('#sg-mgmt').value) }); }
...
/* 관리비: 마지막 다음 달(처음엔 4월)로 날짜만 잡아 폼을 열어줌 → 금액만 입력 */
