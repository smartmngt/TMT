/* =============================================================================
   ...
   - v0817.9: 탭 자가치유(nav 준비까지 재시도 + 앱 복귀 시 재부착) — 배포 후 탭 사라짐 방지
   - v0817.10: 메인 간소화(취득일·증여가액만) · 증여세 세무사수수료 · 바깥터치/안드back → 이전 시트
   ============================================================================= */
(function(){
"use strict";
const $q = s=>document.querySelector(s);

/* 시트 뒤로가기: 바깥 터치/안드로이드 back → 이전 시트(부모)로, 없으면 닫기 */
let _sgBack=null;
const _sgRealClose = window.close;
function sgDismiss(){ const b=_sgBack; _sgBack=null; if(typeof b==='function'){ b(); } else { try{ _sgRealClose(); }catch(e){} } }
try{ window.close = function(){ return sgDismiss(); }; }catch(e){}
...
function giftBreakdownHTML(land,bldg,ded,fee){
  fee=+fee||0;
  if(land+bldg<=0 && fee<=0) return '<span style="color:var(--muted);font-weight:600">토지·건물가액을 넣으면 증여세가 자동 계산돼요.</span>';
  const c=calcGift(land,bldg,ded), w=n=>fmtKRW(n), g=x=>`<b style="color:var(--green)">${x}</b>`;
  const bd='1px solid rgba(23,164,92,.22)';
  let html=`<div style="font-weight:800;font-size:13px;margin-bottom:6px">증여재산가액 = 토지 ${w(land)} + 건물 ${w(bldg)} = ${g(w(c.val))}</div>
  ...
  if(fee>0) html+=`<div style="border-top:${bd};padding-top:8px;margin-top:8px;font-weight:800">+ 세무사 수수료 ${w(fee)} → 증여 총비용 ${g(w(c.tax+fee))}</div>`;
  html+=`<div style="font-size:10.5px;color:var(--muted);font-weight:600;margin-top:8px;line-height:1.5">※ 2026 현행 · 시가인정액·10년내 기납부 증여 등에 따라 실제와 다름. 세무사 확인.</div>`;
  return html;
}
...
function openAddr(){
  ...
  const back=$q('#sg-addrback'); if(back) back.onclick=sgDismiss;
...
function viewSangga(){
  ...
  if(!col.acq){
    const acqRows=[
      ['취득일', a.acqDate||'-'],
      ['증여가액(신고)', a.declared?fmtKRW(+a.declared):'-'],
    ];
  ...
  html+=`<div style="text-align:center;color:var(--muted);font-size:11px;font-weight:600;margin:10px 0 18px">상가 관리 · v0817.10</div>`;
  return html;
}
...
function openInfoView(){
  ...
  const burden=(+a.giftTax||0)+(+a.giftFee||0)+(+a.acqGrand|| +a.acqTax||0);
  window.sheet(`<h2>상가 정보 · 취득(증여)</h2>
    <div class="setcard">
    ...
    <div class="setcard">
      ...
      ${a.giftFee?kv('증여 세무사 수수료', fmtKRW(+a.giftFee)):''}
...
function writeGiftTax(){
  const s=S(), a=s.acq||{};
  const gl=numOf($q('#gt-land').value), gb=numOf($q('#gt-bldg').value), ded=numOf($q('#gt-ded').value), fee=numOf($q('#gt-fee').value);
  const g=calcGift(gl,gb,ded);
  s.acq=Object.assign({}, a, { giftLand:gl, giftBldg:gb, giftDeduction:ded, giftFee:fee, declared:g.val, giftTax:g.tax });
}
function openGiftTax(back){
  back = back||openInfo; _sgBack=back;
  const s=S(), a=s.acq||{};
  ...
  window.sheet(`<h2>증여세 계산</h2><div class="hint">증여재산가액(시가인정액/감정가) 기준. 토지·건물가를 넣으면 자동 계산돼요.</div>
    ...
    <div class="field"><label>세무사 수수료(원)</label><input id="gt-fee" type="text" inputmode="numeric" value="${commaFmt(a.giftFee)}"></div>
    <div class="calc-preview on sg-acqbox" id="gt-calc">${giftBreakdownHTML(numOf(gl),numOf(gb),ded0,numOf(a.giftFee))}</div>
  ...
  ['gt-land','gt-bldg','gt-ded','gt-fee'].forEach(id=>wireComma($q('#'+id)));
  const recalc=()=>{ $q('#gt-calc').innerHTML=giftBreakdownHTML(numOf($q('#gt-land').value),numOf($q('#gt-bldg').value),numOf($q('#gt-ded').value),numOf($q('#gt-fee').value)); };
  ['gt-land','gt-bldg','gt-ded','gt-fee'].forEach(id=>$q('#'+id).addEventListener('input',recalc));
  $q('#gt-cancel').onclick=sgDismiss;
  $q('#gt-save').onclick=()=>{ writeGiftTax(); save(); sgDismiss(); toast('증여세 저장됨'); };
}
...
function openAcqTax(back){
  ...
  $q('#at-cancel').onclick=sgDismiss;
  $q('#at-save').onclick=()=>{ writeAcqTax(); save(); sgDismiss(); toast('취득세 저장됨'); };
}
...
function hookBackup(){
  if(window.__sgHook) return; window.__sgHook=true;
...
/* ---------- 부착 (자가치유: nav 준비까지 재시도 + 앱 복귀 시 재부착) ---------- */
let _wrapped=false, _restored=false;
function wrapRender(){
  if(_wrapped) return; _wrapped=true;
  const orig=window.render;
  window.render=function(){
    if(tab!=='sangga'){ try{ localStorage.setItem('tmt.ui.sangga','0'); }catch(e){} return orig.apply(this, arguments); }
    try{
...
function addNavButton(){
  ...
  if(nav.querySelector('[data-tab="sangga"]')) return true; // 이미 붙어 있음
...
function restoreTab(){
  if(_restored) return;
  try{ if(localStorage.getItem('tmt.ui.sangga')==='1'){ _restored=true; tab='sangga'; window.render(); } }catch(e){}
}
function attach(){
  try{ injectCSS(); hookBackup(); wrapRender(); }catch(e){ console.error('[sangga] init',e); }
  if(addNavButton()){ restoreTab(); return true; }
  return false;
}
function ensureAttached(tries){
  let ok=false;
  try{ ok=attach(); }catch(err){ console.error('[sangga] attach',err); }
  if(!ok && tries>0) setTimeout(()=>ensureAttached(tries-1), 250);
}
if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', ()=>ensureAttached(40));
else ensureAttached(40);
/* 앱을 다시 열거나(bfcache) 포그라운드 복귀 시 탭이 없으면 다시 붙임 */
window.addEventListener('pageshow', ()=>{ try{ addNavButton(); }catch(e){} });
document.addEventListener('visibilitychange', ()=>{ if(!document.hidden){ try{ addNavButton(); }catch(e){} } });
